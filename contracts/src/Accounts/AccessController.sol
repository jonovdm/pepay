...
abstract contract AccessController is IAccessController {
    ...
    struct NewOwnerProposal {
        bytes newSignerProposed;
        uint256[2] pubKeyCoordinates;
        bool resolved;
    ...
    modifier onlyOwnerOrGuardian() {
        require(isOwner(msg.sender) || isGuardian(msg.sender), "ACL:: only owner or guardian");
        _;
    }
        ...
        )
    {
        ...
        return (
            proposal.newSignerProposed,
            proposal.pubKeyCoordinates,
    ...
    function discardCurrentProposal() external onlyOwnerOrGuardian {
        require(!proposals[proposalId].resolved, "ACL:: proposal already resolved");
        if (isGuardian(msg.sender) && proposalTimelock > 0) {
            require(
                (proposals[proposalId].proposedAt + proposalTimelock) < block.timestamp,
                "ACL:: guardian cannot discard proposal until timelock relased"
            );
        }
        if (isGuardian(msg.sender) && proposalTimelock == 0) {
            require(
                (proposals[proposalId].proposedAt + INITIAL_PROPOSAL_TIMELOCK) < block.timestamp,
                "ACL:: guardian cannot discard proposal until timelock relased"
            );
        }
    ...
    function guardianPropose(bytes calldata _credId, uint256[2] calldata pubKeyCoordinates) external onlyGuardian {
        require(guardianCount >= 3, "ACL:: not enough guardians to propose new owner (minimum 3)");
        if (proposals[proposalId].guardiansApproved.length != 0 && proposals[proposalId].resolved == false) {
            revert("ACL:: latest proposal not yet resolved");
        }

        proposalId = proposalId + 1;
        proposals[proposalId].newSignerProposed = _credId;
        proposals[proposalId].pubKeyCoordinates = pubKeyCoordinates;
        ...
        emit ProposalSubmitted(proposalId, _credId, msg.sender);
    }
    ...
    function guardianCosign() external onlyGuardian {
        ...
        proposals[proposalId].guardiansApproved.push(msg.sender);
        proposals[proposalId].approvalCount += 1;
        bytes memory credId = proposals[proposalId].newSignerProposed;
        uint256[2] memory pubKeyCoordinates = proposals[proposalId].pubKeyCoordinates;
        if (_checkQuorumReached(proposalId)) {
            proposals[proposalId].resolved = true;
            // _addOwner(newOwner);
            _addSigner(credId, pubKeyCoordinates);
        } else {
            emit QuorumNotReached(proposalId, credId, proposals[proposalId].approvalCount);
        }
    }
    ...
    function _checkIfSigned(uint256 _proposalId) internal view returns (bool) {
        for (uint256 i; i < proposals[_proposalId].guardiansApproved.length; i++) {
    ...
    function _checkQuorumReached(uint256 _proposalId) internal view returns (bool) {
        return ((proposals[_proposalId].approvalCount * MULTIPLY_FACTOR) / guardianCount >= SIXTY_PERCENT);
    }
}
